...
/** Small copies for home-screen widgets (see scripts/generate-icons.mjs). */
export function eventArtThumbSrc(kind: EventArtKind, variant: "side" | "header"): string {
  return `/event-art/thumbs/${ART_FILE[kind]}-${variant}.jpg?v=illust10`;
}

/** Same choice the app makes: Microsoft events use the business art. */
export function eventArtKindFor(input: {
  summary?: string | null;
  description?: string | null;
  calendarSummary?: string | null;
  eventType?: string | null;
  ...
  if (input.source === "microsoft") return "business";
  return eventArtKind(input);
}
